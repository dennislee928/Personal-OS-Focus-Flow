...
A hybrid GTD × Ivy Lee × Kanban desktop app for daily planning, deep work, and weekly reviews.
...
Capture (GTD) inbox sweep.

Clarify (GTD) w/ 2-minute rule.

Prioritize (Ivy Lee): pick exactly six tasks and order them.

Time-plan: block first 2–3 focus blocks; default 25/5 Pomodoro.

Start with the Frog during 09:00–10:30.
References: GTD 5 steps; Ivy-Lee’s six-task method; Pomodoro steps; Deep Work mornings. 
Cal Newport
+4
WIRED
+4
...
(You can move this to electron-builder.yml if preferred.) 
electron.build

Code Signing & Notarization (must-know)

macOS: Sign with Developer ID Application cert, then notarize and optionally staple the ticket to your app/DMG. Users expect this to run without Gatekeeper warnings. 
Apple Developer
+1

Windows: Sign with a code-signing certificate (EV recommended for SmartScreen reputation). CLI: SignTool or vendor tools; integrate in CI. 
Microsoft Learn
+2
advancedinstaller.com
+2

Notes and gotchas: building Windows installers on macOS is unreliable in practice; prefer native runners (or CI with per-OS workers). 
GitHub
+1
...
Install Kiro, create project, add specs/ and docs/ folders, write initial specs (below). 
Kiro
...
Docs You Can Cite Internally

GTD overview (5 steps). 
WIRED

Ivy Lee (six tasks, daily). 
James Clear
+1
...
Pomodoro (official site, six steps). 
Pomodoro® Technique
+1
...
Kanban/WIP basics (used conceptually in board design). 
Asana

Kiro IDE (spec-driven dev + downloads). 
Kiro
+1

Electron-builder multi-platform distribution & config. 
electron.build
+2
electron.build
+2
