 ...
 */
import { TaskSelectionController } from '../controllers/task-selection-controller.js';
...
export class TaskSelectionInterface {
    ...
    `;
        ...
        this.elements.filterInput = this.elements.root.querySelector('.filter-input');
        this.elements.sortSelect = this.elements.root.querySelector('.sort-select');
        ...
        this.elements.completeButton = this.elements.root.querySelector('.complete-button');
    ...
    setupEventListeners() {
        ...
        this.elements.sortSelect?.addEventListener('change', (e) => {
            const target = e.target;
            this.controller.setSorting(target.value);
        });
        // Clear button
    ...
    render() {
        if (!this.elements.root || !this.state.isVisible)
            return;
    ...
    setupDragDrop() {
        if (!this.elements.selectedTasksList)
            return;
    ...
    renderTaskList() {
      ...
      `;
        ...
        this.elements.taskList.querySelectorAll('.task-item').forEach(item => {
            ...
            const selectBtn = item.querySelector('.select-task-btn');
            const handleSelect = () => {
    ...
    `).join('');
        ...
        this.elements.suggestions.querySelectorAll('.accept-suggestion-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const taskId = e.target.closest('.suggestion-item').getAttribute('data-task-id');
    ...
    renderSelectionCounter() {
        ...
        if (selectedCountEl)
            selectedCountEl.textContent = this.state.selectedCount.toString();
        if (maxCountEl)
            maxCountEl.textContent = this.state.maxTasks.toString();
    }
    renderErrors() {
    ...
    toggleWorkloadAnalysis() {
        const analysis = this.controller.getWorkloadAnalysis();
        const dependencies = this.controller.getUIState().selectedTaskIds.map(id => this.controller.getTaskDependencies(id));
