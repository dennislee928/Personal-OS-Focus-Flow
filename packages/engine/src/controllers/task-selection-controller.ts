...
import { Task, TaskSelectionState } from '../types/ritual.js';
import { TaskSelectionService, TaskSelectionServiceConfig } from '../services/task-selection-service.js';
...
export class TaskSelectionController {
  ...
  private events: TaskSelectionControllerEvents;
  private config: TaskSelectionControllerConfig;
  private uiState: TaskSelectionUIState;
   ...
   */
  getTaskOrder(): Array<{ taskId: string; position: number; title: string }> {
  ...
  hasDependencyIssues(): boolean {
    ...
    for (const task of selectedTasks) {
      const unmetDependencies = task.dependencies.filter(depId => 
        !this.uiState.selectedTaskIds.includes(depId)
      );
