...
export class TaskSelectionService {
  private constraints: TaskSelectionConstraints;
  private priorityWeights: TaskSelectionServiceConfig['priorityWeights'];
  ...
  calculatePriorityScores(tasks: Task[], selectedTaskIds: string[] = []): TaskPriorityScore[] {
    ...
    return tasks.map(task => {
      ...
      const score = Object.entries(factors).reduce((total, [key, value]) => {
        const weight = this.priorityWeights![key as keyof typeof factors];
  ...
  getSuggestions(tasks: Task[], selectedTaskIds: string[]): Task[] {
    ...
    const filteredTasks = availableTasks.filter(task => {
      ...
      const hasUnmetDependencies = task.dependencies.some(depId => 
        !selectedTaskIds.includes(depId) && !tasks.some(t => t.id === depId)
      );
  ...
  detectDependencies(tasks: Task[], targetTaskId: string): {
    ...
    const dependents = tasks.filter(task => 
      task.dependencies.includes(targetTaskId)
    );

    // Check if all prerequisites are available
    const canBeCompleted = targetTask.dependencies.every(depId => 
  ...
  private applyEnergyBasedOrdering(tasks: Task[]): Task[] {
    ...
    for (const task of tasks) {
      ...
      while (queue.length > 0) {
        ...
        const dependenciesMet = current.dependencies.every(depId => 
          processed.has(depId) || !tasks.some(t => t.id === depId)
        );
  ...
  private calculateDueDateUrgency(task: Task): number {
    if (!task.dueDate) return 0.3; // Default medium urgency for tasks without due dates
    ...
    if (daysUntilDue <= 1) return 0.9; // Due today/tomorrow
  ...
  private calculateContextBalance(task: Task, selectedTasks: Task[]): number {
    ...
    if (task.context === '@deep') {
      ...
      if (shallowCount > deepWorkCount + 1) return 1.0;
      if (deepWorkCount >= this.constraints.maxDeepWorkTasks) return 0.0;
      return 0.7;
    } else {
      // Prefer shallow work if we have too many deep tasks
      if (deepWorkCount > shallowCount + 1) return 1.0;
      if (shallowCount >= this.constraints.maxShallowTasks) return 0.0;
      return 0.7;
    }
  }

  private calculateDependencyScore(task: Task, allTasks: Task[], selectedTaskIds: string[]): number {
    ...
    const availableDependencies = unmetDependencies.filter(depId => 
      allTasks.some(t => t.id === depId)
    );
  ...
  private validateDependencies(tasks: Task[], selectedTaskIds: string[]): string[] {
    const errors: string[] = [];
    const selectedTasks = tasks.filter(t => selectedTaskIds.includes(t.id));
